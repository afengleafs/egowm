...
def main(args) :
    ...
    backbone = build_actionfwise2state_svd_model()#VDiffFeatExtractor()
    ...
    #held_out train samples since val navigation samples are not diverse at all
    anno_path = args.anno_path
    val_set = os.listdir(anno_path)
    val_set = ['sample_7870.json', 'sample_26626.json', 'sample_23872.json', 'sample_4567.json', 'sample_8761.json',  'sample_10131.json', 'sample_11103.json'] #nav only
    print(val_set)
    ...
    for traj in tqdm(val_set) :
        ...
        for clip_start in clip_starts :
            ...
            for ridx in realw_idxs :

                init_realw = Image.open(
                    os.path.join(args.realw_dir, f"realw_{ridx}.png")
                ).convert('RGB')

                pred_root = os.path.join(
                    args.val_vis,
                    f"iter_{args.iteration}",
                    f'{traj_name}_{0}_{seg_idx}_{task}',
                ...
                init_realw.save(os.path.join(args.val_vis, f"iter_{args.iteration}", f'{traj_name}_{0}_{seg_idx}_{task}', f"{clip_start}_{clip_end}", f"pred_{ridx}", f"painting_{ridx}.png"), format='PNG')


                image = load_image(init_realw)
                ...
                with torch.no_grad() :
                    ...
                    pred_mp4_path = os.path.join(pred_root, "pred.mp4")
                    pred_video_frames = [init_realw] + output_frames
                    imageio.mimwrite(
                        pred_mp4_path,
...
if __name__ == "__main__" :
    ...
    parser.add_argument(
        ...
        default="output",
        type=str,
        help="Output root directory for logs, models, and visualizations.",
    )
    parser.add_argument(
        "--anno_path",
        default="/data/EVE1x/meta/train",
        type=str,
        help="Path to 1x meta annotations.",
    )
    parser.add_argument(
        "--data_root_1x",
        default="/data/EVE1x/train_v2.0_raw",
        type=str,
        help="Root directory for 1x raw data videos.",
    )
    parser.add_argument(
        "--realw_dir",
        default="/data/cmu_clicks",
        type=str,
        help="Directory containing real-world images (realw_*.png).",
    )
    parser.add_argument(
        "--pretrained_path",
