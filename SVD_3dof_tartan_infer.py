...
def main(args) :
    ...
    test_anno = os.path.join(split_root, "tartan_drive", "test", "traj_names.txt")
    ...
    for traj in tqdm(val_set) :
        ...
        with open(os.path.join(data_root, traj, "traj_data.pkl"), "rb") as f:
        ...
        position_data = traj_data["position"]#fhdf["jackal"]["position"][:, :2]
        yaw_data = traj_data["yaw"]#fhdf["jackal"]["yaw"][()]
        ...
        for clip_start in clip_starts_2 :
            ...
            for t in sample_indx:
                x_y = position_data[t]
                yaw = yaw_data[t]
                positions.append([x_y[0] / METRIC_WAYPOINT_SPACING_TARTAN, x_y[1] / METRIC_WAYPOINT_SPACING_TARTAN])
            ...
            init_frame = Image.open(os.path.join(data_root, traj, f"{init_indx}.jpg")).convert('RGB')
            ...
            gt_dir = os.path.join(args.val_vis, f"iter_{args.iteration}", traj, f"{clip_start}_{clip_end}", "GT")
            if not os.path.exists(gt_dir) :
                os.makedirs(gt_dir)


            pred_dir = os.path.join(args.val_vis, f"iter_{args.iteration}", traj, f"{clip_start}_{clip_end}", "pred")
            ...
            for t in sample_indx[1:] :
                ...
                img = Image.open(os.path.join(data_root, traj, f"{t}.jpg")).convert('RGB')
                ...
                action_x_y = to_local_coords(x_y, init_x_y, init_yaw) / METRIC_WAYPOINT_SPACING_TARTAN
            ...
            with torch.no_grad() :
                ...
                if gt_frames:
                    imageio.mimwrite(os.path.join(gt_dir, "gt.mp4"), gt_frames, fps=4)
                if output_frames:
                    pred_frames = [np.array(f) for f in output_frames]
                    imageio.mimwrite(os.path.join(pred_dir, "pred.mp4"), pred_frames, fps=4)
...
if __name__ == "__main__" :
    ...
    parser.add_argument(
        "--data_root",
        default="data/tartan/",
        help="Root directory containing Tartan trajectories.",
    )
