...
def main(args) :
    ...
    test_anno = os.path.join(split_root, "recon", "test", "traj_names.txt")
    ...
    val_set = test_traj_names #randomly selected to validate
    ...
    for traj in tqdm(val_set) :
        ...
        traj_path = os.path.join(data_root, f"{traj}.hdf5")
        fhdf = h5py.File(traj_path, 'r')

        ep_length = len(np.array(fhdf['images']['rgb_left']))

        print("=======ep_length", ep_length)

        position_data = fhdf["jackal"]["position"][:, :2]
        yaw_data = fhdf["jackal"]["yaw"][()]
        ...
        for clip_start in clip_starts :
            ...
            init_frame = Image.open(io.BytesIO(fhdf['images']['rgb_left'][init_indx])).convert('RGB')
            ...
            pred_root = os.path.join(args.val_vis, f"iter_{args.iteration}", traj, f"{clip_start}_{clip_end}", "pred")
            ...
            for t in sample_indx[1:] :
                ...
                img = Image.open(io.BytesIO(fhdf['images']['rgb_left'][t])).convert('RGB')
                ...
                action_x_y = to_local_coords(x_y, init_x_y, init_yaw) / METRIC_WAYPOINT_SPACING
...
if __name__ == "__main__" :
    ...
    parser.add_argument(
        "--data_root",
        default="data/recon/",
        help="Root directory containing recon .hdf5 files.",
    )
