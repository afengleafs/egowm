"""Action + init_state 条件的 SVD UNet 变体。"""
...
class DebugActionUnetFwise2state(UNetSpatioTemporalConditionModel):
    """在 DebugActionUnetFwise2 基础上额外接受 init_state:

    - action:  per-frame 动作序列 -> 和 time token 拼成 all_emb 喂给 resnet
    - state:   单帧的初始状态 -> 直接加到全局 time embedding 上
    动作空间更大 (action_proj 输入 = 25*32=800),对应 25-DoF 导航任务。
    """
    ...
    def __init__(
        ...
        # 32 sinusoidal channels × 25 action dims = 800
        self.action_proj = Timesteps(32, True, downscale_freq_shift=0)
        self.add_action_embedding = TimestepEmbedding(800, time_embed_dim)

        self.state_proj = Timesteps(32, True, downscale_freq_shift=0)
        self.add_state_embedding = TimestepEmbedding(800, time_embed_dim)
    ...
    def forward(
        ...
        # ---- 2. action embedding (per-frame, concat) ----
        ...
        # ---- 3. state embedding (单帧, 加到 global time emb) ----
        b, a = init_state.shape
        state_flat = rearrange(init_state, "b a -> (b a)")
        state_embeds = self.state_proj(state_flat)
        state_embeds = rearrange(state_embeds, "(b a) c -> b (a c)", b=b).to(emb.dtype)
        emb = emb + self.add_state_embedding(state_embeds)
